...
describe('Give up flow', () => {
  ...
  test('CONFIRM_GIVE_UP from giving-up sets status to lost', () => {
    const state = { ...createInitialState('normal'), status: 'giving-up' as const }
  ...
  test('TICK is ignored during giving-up', () => {
    const state = { ...createInitialState('normal'), timeRemaining: 60, status: 'giving-up' as const }
  ...
  test('GIVE_UP is ignored when not playing', () => {
    const state = { ...createInitialState('normal'), status: 'won' as const }
    const result = gameReducer(state, { type: 'GIVE_UP' })
